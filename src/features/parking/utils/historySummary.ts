...
function isBetter(current: DataPoint | undefined, candidate: DataPoint, targetMinutes: number) {
    ...
    return Math.abs(nMinutes - targetMinutes) < Math.abs(cMinutes - targetMinutes);
}
...
export function buildHistorySummary(
    ...
    return {
        ...
        peakOccupancyRate:
            peakOccupancyRate === -Infinity ? 0 : peakOccupancyRate,
